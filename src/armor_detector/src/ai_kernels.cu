...
__global__ void postprocess_kernel_fp32(
    const float * __restrict__ out, int num_det, float conf_th, int detect_color, float sx,
    float sy, PostDet * __restrict__ out_dets, int max_out, int * __restrict__ out_count)
{
...
#pragma unroll
    ...
    pd.color = best_color;
    pd.prob = conf;
    pd.score_num = best_num_val;
    out_dets[widx] = pd;
}

void launch_postprocess_fp32(
    const float * out_dev, int num_det, float conf_th, int detect_color, float sx, float sy,
    PostDet * out_dets_dev, int max_out, int * out_count_dev, cudaStream_t stream)
{
    int threads = 256;
    int blocks = (num_det + threads - 1) / threads;
    postprocess_kernel_fp32<<<blocks, threads, 0, stream>>>(
        out_dev, num_det, conf_th, detect_color, sx, sy, out_dets_dev, max_out, out_count_dev);
}

static inline dim3 make_block() { return dim3(16, 16, 1); }
...
__global__ void resize_rgb8_to_rgb_nchw_fp16_kernel(
    const unsigned char * __restrict__ src, size_t src_pitch, int srcW, int srcH,
    __half * __restrict__ dst, int dstW, int dstH)
{
...
void launch_resize_rgb8_to_rgb_nchw_fp16(
    const unsigned char * src_rgb_dev, size_t src_pitch, int srcW, int srcH, __half * dst_dev,
    int dstW, int dstH, cudaStream_t stream)
{
    dim3 block = make_block();
    dim3 grid((dstW + block.x - 1) / block.x, (dstH + block.y - 1) / block.y, 1);
    resize_rgb8_to_rgb_nchw_fp16_kernel<<<grid, block, 0, stream>>>(
        src_rgb_dev, src_pitch, srcW, srcH, dst_dev, dstW, dstH);
}
