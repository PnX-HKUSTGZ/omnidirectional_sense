...
void AIDetector::infer(const cv::cuda::GpuMat & gpu_rgb8, int detect_color)
{
    ...
    launch_resize_rgb8_to_rgb_nchw_fp16(
        ...
        static_cast<size_t>(gpu_rgb8.step), gpu_rgb8.cols, gpu_rgb8.rows,
    ...
    launch_postprocess_fp32(
        static_cast<const float *>(output_device_buffer_), num_det, conf_threshold_, detect_color,
        sx, sy, static_cast<PostDet *>(device_post_dets_), max_post_out_, device_post_count_,
        stream_);
    ...
    if (host_count > 0) {
        checkCuda(
            cudaMemcpyAsync(
                ...
                static_cast<size_t>(host_count) * sizeof(PostDet), cudaMemcpyDeviceToHost, stream_),
            "Memcpy dets D2H");
        cudaStreamSynchronize(stream_);
    }
    ...
    for (int i = 0; i < host_count; ++i) {
        ...
        obj.rect = cv::Rect(
            static_cast<int>(d.x), static_cast<int>(d.y), static_cast<int>(d.w),
            static_cast<int>(d.h));
