...
// 头文件不做条件编译，实际可用性由实现文件中的 is_supported() 和 open()
// 返回值决定。
class NvdecMjpegDecoder {
  public:
    ...
    bool open(const std::string &video_device, int width, int height,
              double fps);

    // 读取一帧并在 GPU 上输出 RGB 格式；成功返回 true。
    // out_rgb 会被就地写入/重用。
    // 如 capture_time 非空，则返回底层 V4L2 时间戳；timestamp_monotonic
    // 指示是否基于 monotonic clock。
    bool read_rgb(cv::cuda::GpuMat &out_rgb,
                  struct timeval *capture_time = nullptr,
