...
__global__ void gpuConvertYUYVtoRGB_kernel(
    unsigned char * src, unsigned char * dst, unsigned int width, unsigned int height)
{
    ...
    for (int i = 0; i < height; ++i) {
        ...
        dst[i * width * 3 + idx * 6 + 1] =
            clamp(1.164f * (y0 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), 0.0f, 255.0f);
        ...
        dst[i * width * 3 + idx * 6 + 4] =
            clamp(1.164f * (y1 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), 0.0f, 255.0f);
        dst[i * width * 3 + idx * 6 + 5] =
            clamp(1.164f * (y1 - 16) + 2.018f * (cb - 128), 0.0f, 255.0f);
    }
}

__global__ void gpuConvertYUV422MToRGB_kernel(
    const unsigned char * y_plane, const unsigned char * u_plane, const unsigned char * v_plane,
    unsigned char * dst, size_t y_pitch, size_t u_pitch, size_t v_pitch, size_t dst_pitch,
    unsigned int width, unsigned int height)
{
    ...
    dst_row[0] =
        static_cast<unsigned char>(clamp(1.164f * (y0 - 16) + 1.596f * (cr - 128), 0.0f, 255.0f));
    dst_row[1] = static_cast<unsigned char>(
        clamp(1.164f * (y0 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), 0.0f, 255.0f));
    dst_row[2] =
        static_cast<unsigned char>(clamp(1.164f * (y0 - 16) + 2.018f * (cb - 128), 0.0f, 255.0f));

    if (x_pair + 1 < width) {
        int y1 = y_row[x_pair + 1];
        dst_row[3] = static_cast<unsigned char>(
            clamp(1.164f * (y1 - 16) + 1.596f * (cr - 128), 0.0f, 255.0f));
        dst_row[4] = static_cast<unsigned char>(
            clamp(1.164f * (y1 - 16) - 0.813f * (cr - 128) - 0.391f * (cb - 128), 0.0f, 255.0f));
        dst_row[5] = static_cast<unsigned char>(
            clamp(1.164f * (y1 - 16) + 2.018f * (cb - 128), 0.0f, 255.0f));
    }
}
    ...
    unsigned char * src, unsigned char * dst, unsigned int width, unsigned int height)
{
    ...
    unsigned int flags;
    bool srcIsMapped =
        (cudaHostGetFlags(&flags, src) == cudaSuccess) && (flags & cudaHostAllocMapped);
    bool dstIsMapped =
        (cudaHostGetFlags(&flags, dst) == cudaSuccess) && (flags & cudaHostAllocMapped);
    ...
    unsigned int width, unsigned int height, cudaStream_t stream)
{
    ...
    dim3 grid((pairs + block.x - 1) / block.x, (height + block.y - 1) / block.y);
    gpuConvertYUV422MToRGB_kernel<<<grid, block, 0, stream>>>(
        y_plane, u_plane, v_plane, dst, y_pitch, u_pitch, v_pitch, dst_pitch, width, height);
