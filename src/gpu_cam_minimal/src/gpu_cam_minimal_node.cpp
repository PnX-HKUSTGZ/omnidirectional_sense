...
class GpuCamMinimalNode : public rclcpp::Node {
  public:
    explicit GpuCamMinimalNode(
        const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
        : Node("gpu_cam_minimal", options) {
        // Parameters
        camera_name_ =
            this->declare_parameter<std::string>("camera_name", "cam_0");
        camera_info_url_ =
        ...
        control_params_.brightness =
            this->declare_parameter<int>("brightness", 0);
        control_params_.contrast = this->declare_parameter<int>("contrast", 32);
        control_params_.saturation =
            this->declare_parameter<int>("saturation", 64);
        ...
        if (publish_mode_ == "gpu") {
            gpu_image_pub_ = this->create_publisher<armor_detector::GpuImage>(
        ...
        // Timer at ~fps
        auto period_ms = (framerate_ > 0.0)
                             ? static_cast<int>(1000.0 / framerate_)
                             : 33; // default ~30fps
        timer_ =
  ...
  private:
    ...
    void initializeCudaDevice() {
        ...
        if (err != cudaSuccess) {
            RCLCPP_FATAL(get_logger(), "cudaSetDevice(%d) failed: %s",
                         cuda_device_id_, cudaGetErrorString(err));
            throw std::runtime_error(
                "Failed to initialize CUDA device context");
        }
    }
    void openCamera() {
        ...
        if (use_hw_mjpeg_) {
            ...
            if (!nvdec_->open(video_device_, image_width_, image_height_,
                              framerate_)) {
                RCLCPP_WARN(
                    get_logger(),
                    "Falling back to OpenCV VideoCapture; NVDEC open failed");
                use_hw_mjpeg_ = false;
                nvdec_.reset();
            } else {
                RCLCPP_INFO(get_logger(),
                            "Using Jetson NVDEC MJPEG hardware decode path");
            }
        }
        ...
        if (!use_hw_mjpeg_) {
            ...
            RCLCPP_INFO(
                get_logger(),
                "OpenCV CUDA detected: using Jetson NVDEC path for MJPEG");
        }
    ...
    void apply_camera_controls() {
        ...
        if (fd < 0) {
            RCLCPP_WARN(get_logger(),
                        "Failed to open %s for V4L2 control setup: %s",
        ...
        auto set_ctrl = [&](const char *name, int control_id, int value) {
            ...
            if (::ioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
                if (errno == EINVAL || errno == ENOTTY) {
                    RCLCPP_DEBUG(get_logger(), "Control %s not supported: %s",
                                 name, std::strerror(errno));
                } else {
                    RCLCPP_WARN(get_logger(), "Failed to set %s to %d: %s",
        ...
        if (!control_params_.white_balance_automatic) {
            set_ctrl("white_balance_temperature",
        ...
        set_ctrl("backlight_compensation", V4L2_CID_BACKLIGHT_COMPENSATION,
                 control_params_.backlight_compensation);
        set_ctrl("auto_exposure", V4L2_CID_EXPOSURE_AUTO,
                 control_params_.auto_exposure);
    ...
    rclcpp::Time convert_v4l2_timestamp(const timeval &tv, bool is_monotonic) {
        ...
        rclcpp::Time ref_clock =
            is_monotonic ? steady_clock_.now() : system_clock_.now();
    ...
    void tick() {
        ...
        if (use_hw_mjpeg_) {
            ...
            if (!nvdec_ ||
                !nvdec_->read_rgb(gpu_rgb, &capture_tv, &ts_monotonic)) {
                int err = errno;
                handleNvdecFailure("read/decode failed (errno=" +
            ...
            if (frame_rgb_cpu.empty()) {
                RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                                     "Converted RGB frame is empty");
                return;
            }
            if (d_frame_rgb_.empty() ||
                d_frame_rgb_.rows != frame_rgb_cpu.rows ||
                d_frame_rgb_.cols != frame_rgb_cpu.cols ||
                d_frame_rgb_.type() != frame_rgb_cpu.type()) {
                d_frame_rgb_.release();
                d_frame_rgb_ = cv::cuda::GpuMat(frame_rgb_cpu.size(),
                                                frame_rgb_cpu.type());
            }
        ...
        auto publish_debug_if_requested =
        ...
        if ((publish_mode_ == "gpu" || publish_mode_ == "gpu_hw") &&
            gpu_image_pub_) {
        ...
        const bool cpu_image_required =
            image_pub_ && image_pub_->get_subscription_count() > 0;
        const bool cpu_info_required =
            cam_info_pub_ && cam_info_pub_->get_subscription_count() > 0;
        if (!cpu_image_required && !cpu_info_required) {
            publish_debug_if_requested(ci);
            return;
        }
        ...
        if (!use_hw_mjpeg_) {
            ...
            size_t size_bytes =
                frame_rgb_cpu.total() * frame_rgb_cpu.elemSize();
            msg.data.resize(size_bytes);
            std::memcpy(msg.data.data(), frame_rgb_cpu.data, size_bytes);
        } else {
            // Download minimal copy to publish when GPU transport is not
            // available
            cv::Mat rgb_cpu;
        ...
        if (cpu_image_required) {
            image_pub_->publish(msg);
        }
        if (cpu_info_required) {
...
bool GpuCamMinimalNode::openCpuCapture() {
    ...
    if (!cap_.isOpened()) {
        RCLCPP_ERROR(get_logger(), "Failed to open camera device %s (id=%d)",
                     video_device_.c_str(), device_id);
        return false;
    }

    if (pixel_format_ == "mjpeg" || pixel_format_ == "MJPG") {
        bool ok = cap_.set(cv::CAP_PROP_FOURCC,
                           cv::VideoWriter::fourcc('M', 'J', 'P', 'G'));
        RCLCPP_INFO(get_logger(), "Request MJPG pixel format: %s",
                    ok ? "OK" : "Not supported by backend");
    } else {
        RCLCPP_WARN(get_logger(),
    ...
    RCLCPP_INFO(get_logger(), "Camera opened (OpenCV): %dx%d @ %.1f fps (%s)",
                image_width_, image_height_, framerate_, video_device_.c_str());
    RCLCPP_INFO(get_logger(),
                "OpenCV CUDA detected: will upload frames to GPU (mode=%s)",
                publish_mode_.c_str());
    return true;
}
...
void GpuCamMinimalNode::handleNvdecFailure(const std::string &reason) {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                         "NVDEC failure (%d/%d): %s", nvdec_failure_count_,
                         kNvdecFailureThreshold, reason.c_str());
    if (nvdec_failure_count_ >= kNvdecFailureThreshold &&
        kNvdecFailureThreshold != -1) {
        fallbackToCpuCapture("Exceeded NVDEC failure threshold. Last error: " +
                             reason);
    }
}
...
void GpuCamMinimalNode::fallbackToCpuCapture(const std::string &reason) {
    ...
    RCLCPP_ERROR(get_logger(),
                 "Disabling NVDEC path: %s. Falling back to OpenCV pipeline.",
                 reason.c_str());
    if (nvdec_) {
    ...
    if (!openCpuCapture()) {
        RCLCPP_FATAL(get_logger(),
                     "CPU fallback failed to open device %s; shutting down.",
                     video_device_.c_str());
        rclcpp::shutdown();
    }
}
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        RCLCPP_FATAL(rclcpp::get_logger("gpu_cam_minimal"), "Exception: %s",
                     e.what());
        rclcpp::shutdown();
        return 1;
    }
