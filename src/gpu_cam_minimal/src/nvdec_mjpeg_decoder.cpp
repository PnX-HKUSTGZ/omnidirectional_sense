...
bool NvdecMjpegDecoder::open(const std::string &video_device, int width,
    ...
    if (impl_->v4l2_fd < 0) {
        ...
        RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                "Failed to open V4L2 device %s: %s",
    ...
    if (!impl_->initEglExtensions()) {
        RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                "Failed to initialize EGL KHR extensions");
        return false;
    }

    // 尽力设置分辨率
    (void)NvdecMjpegDecoderImpl::set_v4l2_mjpeg(impl_->v4l2_fd, width, height,
                                                fps);
    ...
    if (v4l2_ioctl(impl_->v4l2_fd, VIDIOC_QUERYCAP, &cap) == 0) {
        if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
            RCUTILS_LOG_ERROR_NAMED(
                "nvdec_mjpeg_decoder",
                "V4L2 device does not support STREAMING API");
            close_decoder();
            return false;
        }
    }
    ...
    // 初始化 V4L2 MMAP 缓冲并开启 STREAMON
    {
        ...
        if (req.count < 2) {
            RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                    "Insufficient V4L2 buffers allocated: %u",
                                    req.count);
            close_decoder();
            return false;
        }
        impl_->v4l2_bufs.resize(req.count);
        impl_->v4l2_buf_fds.assign(req.count, -1);
        bool all_dmabuf_exported = true;
        for (uint32_t i = 0; i < req.count; ++i) {
            ...
            void *start = mmap(nullptr, buf.length, PROT_READ | PROT_WRITE,
                               MAP_SHARED, impl_->v4l2_fd, buf.m.offset);
            if (start == MAP_FAILED) {
                RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
            ...
            exp.index = i;
            exp.flags = O_CLOEXEC;
            if (v4l2_ioctl(impl_->v4l2_fd, VIDIOC_EXPBUF, &exp) == 0) {
                impl_->v4l2_buf_fds[i] = exp.fd;
            } else {
                all_dmabuf_exported = false;
        ...
        if (!all_dmabuf_exported) {
            for (int &fd : impl_->v4l2_buf_fds) {
                if (fd >= 0) {
                    ::close(fd);
                    fd = -1;
                }
        ...
        impl_->camera_dmabuf_enabled = all_dmabuf_exported;
        v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (v4l2_ioctl(impl_->v4l2_fd, VIDIOC_STREAMON, &type) < 0) {
            RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                    "VIDIOC_STREAMON failed: %s",
                                    strerror(errno));
            close_decoder();
            return false;
        }
    ...
    if (impl_->dec->subscribeEvent(V4L2_EVENT_RESOLUTION_CHANGE, 0, 0) < 0) {
        RCUTILS_LOG_WARN_NAMED("nvdec_mjpeg_decoder",
                               "subscribeEvent(V4L2_EVENT_RESOLUTION_CHANGE) "
                               "failed; will use fallback without event.");
    }

    // 设置 OUTPUT 平面格式（输入单帧 JPEG 码流）
    // 使用 V4L2_PIX_FMT_MJPEG 能在部分 Jetson NVDEC
    // 版本上更稳定触发内部解析，避免后续 capture_plane.getFormat EINVAL。
    if (impl_->dec->setOutputPlaneFormat(V4L2_PIX_FMT_MJPEG, 2 * 1024 * 1024) <
        0) {
        RCUTILS_LOG_ERROR_NAMED(
            "nvdec_mjpeg_decoder",
            "setOutputPlaneFormat(V4L2_PIX_FMT_MJPEG) failed");
        close_decoder();
        return false;
    }
    ...
    impl_->output_plane_memory =
        impl_->camera_dmabuf_enabled ? V4L2_MEMORY_DMABUF : V4L2_MEMORY_MMAP;
    uint32_t output_buffer_count =
        impl_->camera_dmabuf_enabled
            ? static_cast<uint32_t>(impl_->v4l2_bufs.size())
            : 2;
    if (impl_->dec->output_plane.setupPlane(
            impl_->output_plane_memory, output_buffer_count, true, false) < 0)
        RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                "output_plane.setupPlane failed");

    // 开启 OUTPUT 流（必须在 capture 前）
    if (impl_->dec->output_plane.setStreamStatus(true) < 0)
        RCUTILS_LOG_ERROR_NAMED("nvdec_mjpeg_decoder",
                                "output_plane.streamon failed");
    else if (impl_->output_plane_memory == V4L2_MEMORY_MMAP) {
        impl_->use_output_reclaim_thread = true;
        impl_->start_output_reclaim_thread();
    } else {
        impl_->use_output_reclaim_thread = false;
    }
...
bool NvdecMjpegDecoder::read_rgb(cv::cuda::GpuMat &out_rgb,
    ...
    if (impl_->v4l2_fd < 0 || !impl_->dec) {
        RCUTILS_LOG_WARN_NAMED("nvdec_mjpeg_decoder",
                               "Invalid decoder or v4l2_fd not opened.");
        return false;
    }
    ...
    if (timestamp_monotonic) {
        *timestamp_monotonic =
            (vbuf.flags & V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC) != 0;
    }
    ...
    if (!impl_->feed_decoder_and_dequeue_capture(vbuf, cam_data, cam_len,
                                                 cap_nvbuf, cbuf)) {
        return false;
    }
    ...
    if (out_rgb.empty()) {
        RCUTILS_LOG_WARN_NAMED("nvdec_mjpeg_decoder",
                               "Output RGB frame is empty after decoding.");
        return false;
    }
...
void NvdecMjpegDecoder::close_decoder() {
    ...
    if (impl_->dec) {
        ...
        if (impl_->use_output_reclaim_thread) {
    ...
    impl_->release_capture_cuda_resources();
    ...
    impl_->v4l2_buf_fds.clear();
    impl_->camera_dmabuf_enabled = false;
